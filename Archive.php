...
// Function to archive old schedules
function archiveOldSchedules() {
    global $conn;
    $currentYear = date('Y');
    $cutoffYear = $currentYear - 1;

    $sql = "INSERT INTO schedule_archive (original_id, branch_id, module_id, salle_id, periode_id, type_seance_id, semester, year, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday)
            SELECT id, branch_id, module_id, salle_id, periode_id, type_seance_id, semester, year, Monday, Tuesday, Wednesday, Thursday, Friday, Saturday
            FROM schedule
            WHERE year <= ?";
    ...
    if ($stmt->affected_rows > 0) {
        // Delete archived schedules from the main table
        $deleteSql = "DELETE FROM schedule WHERE year <= ?";
        $deleteStmt = $conn->prepare($deleteSql);
        $deleteStmt->bind_param("i", $cutoffYear);
        $deleteStmt->execute();
    }
}

// Function to get archived schedules
function getArchivedSchedules($branchId, $semester, $year) {
    global $conn;
    $sql = "SELECT sa.*, m.name AS module_name, GROUP_CONCAT(t.name SEPARATOR ' / ') AS teachers, p.debut_time, p.fin_time, p.name_periode, ts.name AS type_seance_name, sl.name AS salle_name
            FROM schedule_archive sa
            JOIN module m ON sa.module_id = m.id
            JOIN module_teacher mt ON m.id = mt.module_id
            JOIN teacher t ON mt.teacher_id = t.id
            JOIN periode p ON sa.periode_id = p.id
            JOIN type_seance ts ON sa.type_seance_id = ts.id
            JOIN salle sl ON sa.salle_id = sl.id
            WHERE sa.branch_id = ? AND sa.semester = ? AND sa.year = ?
            GROUP BY sa.id, sa.periode_id
...
// Get all years from archived schedules
$yearQuery = "SELECT DISTINCT year FROM schedule_archive ORDER BY year DESC";
...
$schedules = getArchivedSchedules($selectedBranch, $selectedSemester, $selectedYear);
...
<body>
    <div class="container">
        <div class="header">
            <img src="schedule/logoest.png" alt="University Logo" class="logo">
        ...
        <div class="schedule-info">
            Archived Schedule (Year <?php echo $selectedYear; ?>, Semester <?php echo $selectedSemester; ?>) : 
            <?php 
            $selectedBranchName = '';
        ...
        <div class="form-container">
            <form method="get">
                ...
                <input type="submit" value="View Archived Schedule">
            </form>
        </div>
